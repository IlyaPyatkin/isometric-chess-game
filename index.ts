...
const didPieceMove = (state: GameState, position: Position): boolean => {
  return (state.pieces[position] !== initialGameState.pieces[position]) || state.moves.some(({ move }) => move.position === position || move.moveTo === position)
}
...
const stringifyPosition = (position: NumeralPosition): string => {
  return `${columnsString[position.column]}${position.row + 1}`
}

const movePosition = (position: Position, move: RelativeMove): Position | null => {
...
const getPawnMoves = (state: GameState, position: Position): BaseMove[] => {
  ...
  if (forwardPosition) {
    if (!pieces[forwardPosition]) {
      const isPromotionRow = parsePosition(forwardPosition).row === (direction === 1 ? 7 : 0)
      moves.push({ moveTo: forwardPosition,
  ...
  if (diagonalRightPosition) {
    ...
    if (piece && getPieceColor(piece) !== playingColor) moves.push({ moveTo: diagonalRightPosition})
    if (!piece) {
      // todo: add en passant
    }
  }
  ...
  if (diagonalLeftPosition) {
    ...
    if (piece && getPieceColor(piece) !== playingColor) moves.push({ moveTo: diagonalLeftPosition })
    if (!piece) {
      // todo: add en passant
    }
  }
...
function parsePiece(piece: Piece): { color: PieceColor, type: PieceType } {
  return { color: getPieceColor(piece), type: getPieceType(piece) }
}

function getPiecesPositions(state: GameState, color: PieceColor): Position[] {
  return (Object.keys(state.pieces) as Position[]).filter(position => getPieceColor(state.pieces[position]!) === color)
}
...
function getPieceMoves(state: GameState, position: Position): BaseMove[] {
  const playingColor = getPlayingColor(state)
...
const getPlayingColor = (state: GameState): PieceColor => {
...
const getAllAvailableMoves = (state: GameState, color: PieceColor): BaseMove[] => {
  return getPiecesPositions(state, color).flatMap(position => getPieceMoves(state, position))
}

const getAllAvailableMovePositions = (state: GameState, color: PieceColor): Position[] => {
